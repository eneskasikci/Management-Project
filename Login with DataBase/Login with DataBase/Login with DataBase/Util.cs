...
namespace Login_with_DataBase
{
    public static class Util
    {
        ...
        public static void LoadPersonalInformation(List<User> userlist, string csvname, int userindex)
        {
            if (!File.Exists(@csvname))
            {
                StreamWriter create = File.CreateText(@csvname);
            ...
            while (!reader.EndOfStream)
            {
                ...
                {
                    var values = line.Split(',');
                    if (int.Parse(values[0]) == userindex)
                    {
                        LoginForm.userList[userindex].Person.Name = values[1];
                        LoginForm.userList[userindex].Person.Surname = values[2];
                        LoginForm.userList[userindex].Person.Phonenumber = values[3];
                        LoginForm.userList[userindex].Person.Email = values[4];
                        LoginForm.userList[userindex].Person.Address = values[5];
                        LoginForm.userList[userindex].Person.Image = values[6];
                    }
        ...
        public static void savePersonalInformation(List<User> userlist, string csvname, int userindex)
        {
            ...
            for (int i = 0; i < userlist.Count; i++)
            {
                ...
                personalInformationWrite.WriteLine(i + "," + user.Person.Name + "," +
                                         user.Person.Surname + "," + user.Person.Phonenumber + "," +
                                         user.Person.Email + "," + user.Person.Address + ',' + user.Person.Image);
            }
            personalInformationWrite.Close();
        }
        public static void ChangePassword(List<User> userlist, string csvname, int userindex){
            StreamWriter changePasswordWrite = File.CreateText(@csvname);
            for (int i = 0; i < userlist.Count; i++)
            {
                User user = userlist[i];
                if (user.Phonebook.Count != 0)
                {
                    for (int j = 0; j < user.Phonebook.Count; j++)
                    {
                        changePasswordWrite.WriteLine(user.Username + ","+ user.Password + "," +
                                                      user.Phonebook[j].Phonenumber + "," +user.Usertype);
                    }
                }
            }
            changePasswordWrite.Close();
        }
